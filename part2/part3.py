...
def getTrainingData(expNum=2):
    ...
    if expNum ==3:
        ...
        for o in range(0,10):
            tempTrain    = pd.read_csv("Data/Part3_"+str(o)+"_Train.csv").values[0:100]
            tempTest     = pd.read_csv("Data/Part3_"+str(o)+"_Test.csv").values[0:100]
            hotEncode    = np.repeat([0],10)
            hotEncode[o] = 1
            tempTrainLab = np.repeat([hotEncode], tempTrain.shape[0],0)
            tempTestLab  = np.repeat([hotEncode], tempTest.shape[0],0)
...
def evaluateNetwork(epochs,
                    ...
                    numFilters,
                    trial_num):
    train_data, test_data, train_labels, test_labels = getTrainingData(3)
