...
def saveImagesAndTimeLines(numFilters,filterW, trainAccTimeline, trainLossTimeline,
    ...
    if numFilters == 2:
          plt.imshow(filterW[0], cmap="summer")#, interpolation='nearest')
          plt.savefig("Filters/part"+str(kwargs['ExNum'])+"/filter"+str(0)+"_"+SaveString+".jpg")
          plt.imshow(filterW[1], cmap="autumn")#, interpolation='nearest')
          plt.savefig("Filters/part"+str(kwargs['ExNum'])+"/filter"+str(1)+"_"+SaveString+".jpg")
    else:
...
def runNetwork(**kwargs):
  ...
  for epoch in range(0, Epochs):
    ...
    for t in trainIndexList:
      ...
      #print("A0:"+str(np.sum(A[0]))) ##A0 is not changing
      #print("A1:"+str(np.sum(A[1])))
      ###Regularize
      ### 
      #A[0] = A[0]/np.sum(A[0])
      #A[1] = A[1]/np.sum(A[1])
    ...
    #ConvLayer
      ...
      for r in range(0,R):
        for k in range(0,K):
          ...
          dE_V[k]    += dE_dXr[r]*dXr_dZr[r]*dZr_dVk[r,k]   #TODO this is a mistake
      ...
      #    for i in range(0,I):
      #      for j in range(0,J):
      #        #if np.max(X_0[i:C-(I-i)+1, j:D-(J-j)+1]) != 0:
      #        #pdb.set_trace()
      #        dE_dA[l,i,j] = np.sum(X_0[i:C-(I-i)+1, j:D-(J-j)+1])*np.sum(ACT(X_1[l,:,:]))*np.sum(dE_V.reshape(L,M,N)[l,:,:])


      #quicker BackProp
        for i in range(0,I):
          for j in range(0,J):
            dE_dA[:,i,j] = np.sum(X_0[i:C-(I-i)+1, j:D-(J-j)+1])
  
        for l in range(0,L):
          dE_dA[l]*= np.sum(ACT(X_1[l,:,:]))*np.sum(dE_V.reshape(L,M,N)[l,:,:])
      ...
      #apply deltas
      A -= lrA * dE_dA
      if not INIT=="Freeze" or INIT=="FreezeBot" or INIT=="FreezeTop":
      	W -= lrW * dE_dW
